	...
	function tjBaackTopController() {
		...
		if (scrollElementWrap?.length) {
			const scrollPercentage = () => {
				...
				const percentageElement = $("#tj-back-to-top-percentage");

				if (scrollTopPos <= 100) {
					scrollElementWrap.removeClass("active");
					scrollElementWrap.css("background", "");
					percentageElement.empty();
					return;
				}
				...
				const scrollValue =
					calcHeight > 0 ? Math.round((scrollTopPos / calcHeight) * 100) : 0;
				...
				if (scrollValue < 96) {
					percentageElement.text(`${scrollValue}%`);
				} else {
					percentageElement.html('<i class="tji-arrow-up-long"></i>');
				}
			};
